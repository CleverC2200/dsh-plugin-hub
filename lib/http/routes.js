import { desktopControl, hasDesktopOwner } from '../services/desktop-control.js';
import { companyCatalog, companyInstallable } from '../services/company-catalog.js';
/**
...
import { gitLsRemote, probeUrl, systemProxy } from '../services/probe.js';
...
import { isDshPlugin, isEntryLoaded, configuredBundles, pluginRuntimeStatus } from '../services/loader.js';
    ...
    const disposers = [
        webServer.register({ kind: 'exact', path: '/dsh-plugin-hub/desktop-updates', handler: async (request, response) => {
                if (request.method === 'GET') {
                    if (!hasDesktopOwner()) {
                        sendJson(response, 200, { desktop: false });
                        return;
                    }
                    try {
                        sendJson(response, 200, { desktop: true, ...await desktopControl('status') });
                    }
                    catch {
                ...
                try {
                    ...
                    if (!['check', 'settings', 'prepare', 'cancel', 'restart'].includes(body?.action ?? '')) {
                        sendJson(response, 400, { error: 'INVALID_UPDATE_ACTION' });
                        return;
                    }
                    sendJson(response, 200, await desktopControl(body.action, body.value ?? {}));
                }
                catch (error) {
                    sendJson(response, 503, { error: error instanceof Error && /^[A-Z][A-Z_0-9]+$/.test(error.message) ? error.message : 'DESKTOP_UPDATE_FAILED' });
                }
            } }),
        webServer.register({
            kind: 'exact',
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                if (!requireMethod(request, response, 'GET'))
                    return;
                const url = new URL(request.url ?? '/', 'http://localhost');
                sendJson(response, 200, companyCatalog(url.searchParams.get('stats') === '1', url.searchParams.get('lang') ?? 'zh'));
            },
        }),
        webServer.register({
            kind: 'exact',
            ...
            handler: async (request, response) => {
                if (!requireTrustedPost(request, response))
                    return;
                if (hasDesktopOwner()) {
                    sendJson(response, 409, { error: '请使用公司桌面更新 / Use company desktop updates' });
                    return;
                }
                ...
                try {
                    ...
                    const source = typeof body === 'object' && body !== null && typeof body.source === 'string'
                        ? body.source
                        : '';
                    if (source === 'catalog' && !companyInstallable(githubRepoOf(rawRepo) ?? rawRepo)) {
                        sendJson(response, 403, { error: 'Company catalog installation is unavailable for this repository / 此仓库尚未开放目录安装' });
                        return;
                    }
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                if (!requireTrustedPost(request, response))
                    return;
                if (hasDesktopOwner()) {
                    sendJson(response, 409, { error: '桌面插件由外置版本管理 / Desktop plugins use immutable versions' });
                    return;
                }
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                if (hasDesktopOwner()) {
                    try {
                        sendJson(response, 200, await desktopControl('restart', {}));
                    }
                    catch {
                        sendJson(response, 503, { error: 'DESKTOP_RESTART_FAILED' });
                    }
                    return;
                }
