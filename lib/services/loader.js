...
export function isEntryLoaded(loader, name) {
    if (!loader)
        return false;
    for (const entry of Array.from(loader.entries())) {
        if (!entry.options?.disabled && nameMatches(entry.options?.name, name))
            return true;
    }
    return false;
}
...
export function configuredBundles(profile) {
    try {
...
export function pluginRuntimeStatus(profile, name, loader, startupBundles) {
    if (!loader)
        return 'unknown';
